...
function Assignments() {
  ...
  const fetchAssignments = async () => {
    try {
      ...
      const assignmentData = Array.isArray(response) ? response : [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingAssignment) {
        await api.put(
          `/assignments/${editingAssignment.ssn}/${editingAssignment.proj_no}`,
          data
        );
      } else {
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredAssignments.map((assignment) => (
                  <tr
                    key={`${assignment.ssn}-${assignment.proj_no}`}
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div>
                        <div className="text-sm font-medium text-gray-900">
                          {assignment.proj_name}
                        </div>
                        <div className="text-sm text-gray-500">
                          Project #{assignment.proj_no}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end space-x-2">
                        ...
                        <button
                          onClick={() =>
                            handleDelete(assignment.ssn, assignment.proj_no)
                          }
